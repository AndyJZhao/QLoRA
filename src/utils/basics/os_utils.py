...
env_cfg_file = 'configs/proj/env.yaml'
ENV_VARS = OmegaConf.load(env_cfg_file).env.vars


def init_env_variables(env_vars=ENV_VARS):
    for k, v in env_vars.items():
        k = k.upper()
        if k == 'PATH':
            os.environ[k] = f'{v}:{os.environ[k]}'
        else:
            os.environ[k] = v
...
def remove_file_or_path(file_or_path):
...
def floor_quantize(val, to_values):
    # Zhaocheng: please use np.searchsorted or torch.bucketize instead
...
def time_logger(func):
    def wrapper(*args, **kw):
        ...
        logger.info(f'Start running {func.__name__} at {get_cur_time()}')
        ret = func(*args, **kw)
        logger.info(
            f'Finished running {func.__name__} at {get_cur_time()}, running time = '
            f'{time2str(time.time() - start_time)}.')
        return ret
